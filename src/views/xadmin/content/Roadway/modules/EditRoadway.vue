        ...
        <a-col>
          <a-form-item  label="巷道名称" name='a'>
            ...
            <a-input
              v-decorator="[
                'a',
                { initialValue: formList.wkName, rules: [{ required: true, message: '请输入巷道名称' }] },
              ]"
              placeholder="请输入巷道名称"
        ...
        <a-col>
          <a-form-item label="所属采区" >
            <a-select
              v-decorator="[
                'b',
                { initialValue: formList.workingSector, rules: [{ required: true, message: '请选择所属采区' }] },
              ]"
              placeholder="请输入巷道名称"
        ...
        <a-col>
          <a-form-item label="巷道类型" >
            <a-select
              v-decorator="[
                'c',
                { initialValue: formList.type, rules: [{ required: true, message: '请选择巷道类型' }] },
              ]"
              placeholder="请选择巷道类型"
        ...
        <a-col>
          <a-form-item label="容纳人数阈值">
            <a-input
              v-decorator="[
                'd1',
                { initialValue: 5, rules: [{ required: true, message: '请输入容纳人数阈值' }] },
              ]"
...
<script>
import { getEditTunnel, getTunnelInfo } from '@/api/code/allApi'
...
export default {
  ...
  methods: {
    getWorkInfo () {
      getTunnelInfo({ id: this.$route.query.id }).then(res => {
