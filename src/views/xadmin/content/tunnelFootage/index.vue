...
<!--    掘进指标/进尺管理-->
        ...
        <a-col class="table-wrapper">
          <a-table :dataSource="dataSource" :columns="columns" bordered :pagination="false">
            <span slot="action" slot-scope="text, record">
              <a-button type="link" @click="addLog(record)">进尺填报</a-button>
    ...
    >
      <a-form-model :model='addLogPopUp' :rules='rules' ref='form' :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
        ...
        <a-form-model-item label="进尺（m）" prop='FootageNum'>
          <a-input
...
import { TunnelAddFootageLog, TunnelFootageList } from '@/api/code/allApi'
  ...
  data () {
    return {
      ...
      rules: {
        ...
        FootageNum: [{ required: true, message: '请输入进尺', trigger: 'change' },
